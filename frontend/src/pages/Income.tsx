...
export default function Income() {
  ...
  function load() {
    if (!selectedCompany) return
    listTransactions(selectedCompany, 'INCOME').then(setItems).catch((e) => setError((e as Error).message))
  }
  ...
  return (
    <div className="fade-in">
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setError(null) }} title="Add Income Entry">
      ...
      <div className="panel">
        <h3 style={{ marginTop: 0, marginBottom: 20 }}>Income Transactions</h3>
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                items.map((t) => (
                  <tr key={t.id}>
                    <td>{t.date}</td>
                    <td style={{ fontWeight: 600, color: 'var(--success)' }}>₹ {Number(t.amount).toFixed(2)}</td>
                    <td>{t.description || '-'}</td>
                    <td>
